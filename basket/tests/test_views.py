...
class TestBasketView(TestCase):
    def setUp(self):
        # We are creating some data for testing.
        ...
        Product.objects.create(category_id=1, title='django advanced', created_by_id=1,
                               slug='django-beginners', price='20.00', image='django')
        # Added two items to the session.
        self.client.post(
    ...
    def test_basket_add(self):
        ...
        response = self.client.post(
            # Accessing basket URL using namespace.
            reverse('basket:basket_add'), {"productid": 3, "productqty": 1, "action": "post"}, xhr=True)
        # Earlier in setUp we added two items, qty 1,2 so here final we are checking {'qty':4}
        self.assertEqual(response.json(), {'qty': 4})
        # Earlier in setUp {"productid": 2, "productqty": 2, "action": "post"}, xhr=True),
        # now we updated that qty from 2 to 1.Checked does if statement work. 
        response = self.client.post(
